...
class Module extends DependenciesBlock {
	...
	isAccessibleInChunk(chunk, ignoreChunk, assumeAccessible) {
		const queue = new Set([chunk]);
		for(const chunk of queue) {
			if(assumeAccessible !== undefined && assumeAccessible.has(chunk)) continue;
			if(chunk === ignoreChunk || !chunk.containsModule(this)) {
				if(chunk.getNumberOfParents() === 0) return false;
				if(chunk instanceof ChunkGroup) {
					if(!this.isAccessibleInChunkGroup(chunk, ignoreChunk, queue))
						return false;
				} else {
	...
	isAccessibleInChunkGroup(chunkGroup, ignoreChunk, assumeAccessible) {
		for(const chunk of chunkGroup.parentsIterable) {
			if(assumeAccessible !== undefined && assumeAccessible.has(chunk))
				return true;
			if(this.isAccessibleInChunk(chunk, ignoreChunk, assumeAccessible))
				return true;
		}
		return false;
	}
